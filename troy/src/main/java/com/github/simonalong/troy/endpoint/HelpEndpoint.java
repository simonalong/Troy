...
public class HelpEndpoint {
    ...
    public String getList() {
        ...
        apiMessage.append("help 查询：修改ip和port      ").append("curl -X POST -H 'Content-Type: application/json' '" + apiPrefix() + "/help?ip={ip}&port={port}'").append("\n");
        apiMessage.append("======================").append("\n");
        ...
        apiMessage.append("分组 变更：更新分组信息      ").append("curl -X POST -H 'Content-Type: application/json' '" + apiPrefix() + "/group?group={group}&logLevel={logLevel}&enable={enable}'").append("\n");
        apiMessage.append("分组 变更：某个函数更新      ").append("curl -X POST -H 'Content-Type: application/json' '" + apiPrefix() + "/group/fun/change?funId={funId}&logLevel={logLevel}&enable={enable}'").append("\n");
        apiMessage.append("------日志------------").append("\n");
        apiMessage.append("分组 日志查询：分组某个函数信息  ").append("curl '" + apiPrefix() + "/group/fun/info/one/logger?group={group}&logFunId={funId}'").append("\n");
        ...
        apiMessage.append("分组 输出变更：添加分组输出      ").append("curl -X POST -H 'Content-Type: application/json' '" + apiPrefix() + "/group/(console/file/all)?group={group}&logLevel={logLevel}&enable={enable}'").append("\n");
        apiMessage.append("分组 输出变更：添加函数输出      ").append("curl -X POST -H 'Content-Type: application/json' '" + apiPrefix() + "/group/fun/print/(console/file/all)?funId={funId}&logLevel={logLevel}&enable={enable}'").append("\n");
        apiMessage.append("======================").append("\n");
        ...
        apiMessage.append("logger 更新：logger更新级别    ").append("curl -X POST -H 'Content-Type: application/json' '" + apiPrefix() + "/logger?loggerName={loggerName}&logLevel={logLevel}'").append("\n");
        apiMessage.append("======================").append("\n");
        apiMessage.append("appender 更新：添加输出器        ").append("curl -X POST -H 'Content-Type: application/json' '" + apiPrefix() + "/appender/console?loggerName={loggerName}&logLevel={logLevel}'").append("\n");
